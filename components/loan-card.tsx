...
export function LoanCard({ loan }: { loan: LoanView }) {
  ...
  return (
    <Link
      href={`/loans/${loan.id}`}
      className="group block rounded-2xl border border-warm-200 bg-card p-4 transition-[border-color,box-shadow] duration-200 ease-[var(--ease-out-quint)] hover:border-brand-line hover:shadow-[0_10px_28px_-14px_rgba(36,58,138,0.30)] focus-ring"
    >
      ...
      <div className="mt-3 flex items-end justify-between gap-3">
        <div className="min-w-0">
          ...
          />
          <p className="mt-0.5 truncate text-sm text-warm-500">{loan.reason}</p>
        </div>
        {showCountdown && (
          <span
